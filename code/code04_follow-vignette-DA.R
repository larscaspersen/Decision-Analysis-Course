#follow the vignette https://cran.r-project.org/web/packages/decisionSupport/vignettes/example_decision_function.html
...
input <- decisionSupport::estimate_read_csv("data/vignette_example_input_table.csv")
...
#plot net present value distributions
...
#combine boxplot with density
decisionSupport::plot_distributions(mcSimulation_object = mcSimulation_results, 
                                    vars = "NPV_decision_do",
                                    method = 'boxplot_density')

#plot the simulated cashflow
plot_cashflow(mcSimulation_object = mcSimulation_results, cashflow_var_name = "Cashflow_decision_do")

#run pls regression to estimate the importance of parameters
pls_result <- plsr.mcSimulation(object = mcSimulation_results,
                                resultName = names(mcSimulation_results$y)[3], ncomp = 1)


input_table <- read.csv("data/vignette_example_input_table.csv")
plot_pls(pls_result, input = input_table, threshold = 0)
...
compound_figure(mcSimulation_object = mcSimulation_results, input_table = input_table, plsrResults = pls_result, 
                EVPIresults = evpi, decision_var_name = "NPV_decision_do", cashflow_var_name = "Cashflow_decision_do", 
                base_size = 7)
